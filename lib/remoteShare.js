...
let shareData = [];
let appInsightsClient = global.appInsights.defaultClient;
...
app.post('/leafApi', function (req, res) {
    try {
        ...
        switch (msgData.msgType) {
            case global.protos.MESSAGETYPE.SHARE:
                try {
                    ...
                    global.appInsights.trackEvent("shareSubmitFailed");
                    global.appInsights.trackException({exception: e});
                }
                return res.json({'success': true});
            case global.protos.MESSAGETYPE.BLOCK:
                global.database.storeBlock(msgData.exInt, msgData.msg, function(data){
                    if (!data){
                        global.appInsights.trackEvent("blockSubmitFailed");
                        return res.status(400).end();
                    } else {
                        global.appInsights.trackEvent("blockSubmitted");
                        return res.json({'success': true});
                    }
                });
                break;
            case global.protos.MESSAGETYPE.INVALIDSHARE:
                global.database.storeInvalidShare(msgData.msg, function(data){
                    if (!data){
                        global.appInsights.trackEvent("invalidShareSubmitFailed");
                        return res.status(400).end();
                    } else {
                        global.appInsights.trackEvent("invalidShareSubmitted");
                        return res.json({'success': true});
                    }
                });
                break;
        ...
        global.appInsights.trackEvent("invalidWebSocketFrame");
        global.appInsights.trackException(e);
        return res.status(400).end();
    }
});
...
function storeShares(){
    if (Object.keys(shareData).length > 0){
        ...
        global.appInsights.trackEvent("bulkSharesStored");
        shareData = [];
    }
